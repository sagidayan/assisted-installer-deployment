...
def is_logfile_handled(existing_tickets, failure_id):
    ...
    if summary in existing_tickets:
        logger.info(f"Marking {failure_id} as handled due, fount summary {summary}")
        mark_logs_as_handled(marked_path, summary)
        return True

    logger.info(f"Item {existing_tickets} looks new")
    return False
...
def main(arg):
    ...
    for failure in failed_clusters:
        ...
        if cluster['status'] == "error":
            if args.mark_root and is_logfile_handled(summaries, failure['name']):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mark-root", default=None, required=False, help="Location of the logs for marking as "
                                                                             "handheld")
